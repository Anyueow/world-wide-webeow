...
| T-0.5 | Write the biography | DONE | opus-5/014ecdjnxdyE | `research/BIOGRAPHY.md`. Nine sections. The thesis moved from single-source to confirmed: her 2024 job intro says the same thing as her 2026 answer, unprompted, two years apart. Sensitive financial and immigration material was read and deliberately kept out of every committed file, see the SENSITIVE section of the ledger. Two unresolved facts are flagged inline rather than smoothed over. | Blocked on T-0.4. `research/BIOGRAPHY.md`. Not website copy. The long, honest read: origin, the through-line, the contradictions, the drivers, what the pattern across 2015 to 2026 actually says about her. Written as if for a book, so that everything the site says can be traced back to a paragraph here. Ananya's framing: "read and study me like you're writing my autobiography." |
| T-0.6 | Frame the story | DONE | opus-5/014ecdjnxdyE | `research/story-frame.md`. Thesis in one sentence, why the game form is structurally right rather than decorative, the three beats, two draft opening lines, and a row per game symbol saying what it is doing in the story. **The dinner table is the keystone symbol**, it states the thesis as an object. Her three decoys are better than mine and the file says why. | Blocked on T-0.5. One page: the thesis of the site in a sentence, the three beats it moves through, and the specific reason each game symbol is in the game. This is what makes the game mean something rather than being a novelty. Commit as `research/story-frame.md`. |
...
| T-G.11 | Symbols must FLOAT across the whole page | IN PROGRESS | sonnet-5/session_797907ec, 2026-09-04T00:35Z | **Ananya, 2026-09-04, verbatim, this is the spec, recorded here per CLAUDE.md 1a so no other agent has to guess it:** "also i want the symbols to be floating all around the webpage not in a section. and a basket on the bottom right animating that the symbols are collected when clicked on and basket expands and swallows the symbol. When users click on the basket the cards show up with what they've collected and if they are the right elements or not (total score inc.) correct - incorrect symbols." Breaking that into three parts, tracked as one build: (1) icons scattered across the whole document, not boxed in `#game`, (2) clicking one animates it flying into a bottom right basket which visibly swallows it, (3) clicking the basket opens a card view of everything opened so far, marked correct (real, collected) or incorrect (decoy, opened), with a running score. Building now as `GameProvider` (shared context: collected ids, attempted ids, flight state, basket open state, persisted via `game-storage.ts`), `FloatingIcon` (absolutely positioned within whichever section it is placed in, deterministic hash position so no hydration mismatch), `GameToast` (shows the clicked item's reveal since there is no longer one boxed panel to show it in), `FlyingIcon` (the fly-to-basket animation, transform and opacity only, skipped entirely under `prefers-reduced-motion`), `GameBasket` (fixed bottom right, count badge, bump on collect), `Scorecard` (the card view, correct vs incorrect, opens on basket click). `GameBoard.tsx` is being retired once this lands, its mechanic (click a true item to collect, click a decoy to see why it is a joke) carries over unchanged into `FloatingIcon` plus `GameProvider`. |
| T-G.12 | **`main` does not build** | TODO | sonnet-5/session_797907ec (owns the game components) | **Blocking everyone: `npm run build` and `npx tsc --noEmit` both fail on `main` right now, so no agent can meet the definition of done.** Two causes, one already fixed. (1) FIXED by opus-5: `GameBoard.tsx` imported `loadCollectedIds` and `saveCollectedIds` from `@/lib/game-storage`, which exports `loadProgress` and `saveProgress` instead. The import was dead, nothing in the file called either function, so removing the line is a zero-behaviour change and the build error is gone. (2) STILL BROKEN and not mine to fix: `GameBasket.tsx` imports `@/components/game/Scorecard`, which does not exist. `GameBasket` is currently an orphan, nothing imports it, but TypeScript still checks it so the whole build fails. Whoever lands the scorecard fixes this by adding `Scorecard.tsx`. If the scorecard is going somewhere else, delete `GameBasket.tsx`, it is in git history and nothing references it. Left alone deliberately rather than guessing at the scorecard's design mid-flight. |
| T-G.4 | Basket and collection animation | IN PROGRESS | sonnet-5/session_797907ec, 2026-09-04T00:35Z | Folded into T-G.11, same spec, same build. `FlyingIcon.tsx` animates the clicked icon from its click position to the basket's bounding rect (read off a ref on the basket button) and fades and shrinks it, standing in for "the basket swallows it". `prefers-reduced-motion` skips the flight entirely, the count still updates instantly since that state change happens in `GameProvider.collect()` regardless of whether the animation runs. |
| T-G.5 | Hovering scorecard | IN PROGRESS | sonnet-5/session_797907ec, 2026-09-04T00:35Z | Folded into T-G.11, same spec, same build. Renamed in practice to a click-to-open scorecard rather than hover, since Ananya's own wording is "when users click on the basket the cards show up", and hover has no equivalent on a phone (see T-G.8). `Scorecard.tsx` lists every attempted item (opened, whether collected or a decoy) with its reveal text and a correct/incorrect marker, plus a running score line. This also happens to resolve T-G.10 (reveal copy missing from server rendered HTML) for anything shown in the scorecard once opened, though `RevealAll` (T-G.6) is still the one guaranteed always-in-the-HTML copy of every reveal, scorecard or no scorecard, JS or no JS. |
| T-G.6 | Reveal-all escape hatch | DONE | sonnet-5/session_797907ec-sub, 2026-09-04T00:00Z | Built `src/components/game/RevealAll.tsx`, a `<details>/<summary>` disclosure with no client state, rendered right after `<GameBoard />` in the `#game` section of `src/app/page.tsx`. Lists every item from `src/content/game-items.ts` (label plus reveal, decoys marked ", a lie") with no invented copy. Needs zero JS since it is a native element, so its content is in the server rendered HTML from the start; verified with `grep "Skip the game, show me everything" out/index.html` and `grep "<details" out/index.html` after `npm run build`, both hit, and confirmed the item reveal text (e.g. the running shoes line) is also in the static output. `npm run build`, `npx tsc --noEmit`, `npm run lint` all pass (lint has 4 pre-existing warnings in GameBoard.tsx, unrelated to this change, no errors). |
...
| T-G.12 | Hotfix: main broken by stale GameBoard import | DONE | sonnet-5/objective-einstein-a8bdf9, 2026-09-04T00:00Z | `main` did not build: `GameBoard.tsx` imported `loadCollectedIds`/`saveCollectedIds` from `@/lib/game-storage`, but that module only ever exported `loadProgress`/`saveProgress`, and `GameBoard.tsx` never actually called the imported functions anywhere in its body (dead import from an interrupted edit, likely mid-T-G.11). `GameProvider.tsx` already correctly uses `loadProgress`/`saveProgress` (the real, intended persistence path per T-G.9/T-G.11), so the fix was to drop the dead, mismatched import from `GameBoard.tsx` rather than rename the storage module's exports. This specific mismatch is fixed and `npx tsc --noEmit` no longer errors on it. **Two unrelated pre-existing type errors still block a clean `npx tsc --noEmit`/`npm run build`, both inside T-G.11's claimed, in-progress scope, not touched here:** `src/components/game/GameBasket.tsx` imports `@/components/game/Scorecard`, which does not exist yet (tracked under T-G.11/T-G.5, `Scorecard.tsx` not yet built), and `src/app/layout.tsx:68` references an undefined `LayoutProps` type. Whoever picks up T-G.11 next needs to land `Scorecard.tsx` and fix the `layout.tsx` type error before `main` is green again. |
