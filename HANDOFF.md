...
| T-G.3 | The game board | SUPERSEDED | sonnet-5/session_797907ec | Replaced by T-G.11. `GameBoard.tsx` deleted; its mechanic (click a true item to collect, click a decoy to see the joke) lives on in `FloatingIcon.tsx` plus `GameProvider.tsx`. |
| T-G.12 | `main` does not build | DONE | sonnet-5/session_797907ec | Both causes are gone. `GameBoard.tsx` (the file with the stale `loadCollectedIds`/`saveCollectedIds` import) is deleted outright as part of T-G.11, not patched. `GameBasket.tsx`'s import of `@/components/game/Scorecard` now resolves: `Scorecard.tsx` exists, see T-G.11. `npx tsc --noEmit` and `npx eslint src` both clean after this merge. |
| T-G.10 | Reveal copy is not in the server rendered HTML | DONE | sonnet-5/session_797907ec-sub | Fixed by T-G.6's `RevealAll` (`<details>/<summary>`, no JS, every reveal in the static HTML from the start). Confirmed still present after the T-G.11 rewrite: `RevealAll` now renders inside the intro section rather than the old `#game` section, same content, same guarantee. |
| T-G.11 | Symbols FLOAT across the whole page, basket, scorecard | DONE | sonnet-5/session_797907ec | **Ananya, 2026-09-04, verbatim, this is the spec:** "also i want the symbols to be floating all around the webpage not in a section. and a basket on the bottom right animating that the symbols are collected when clicked on and basket expands and swallows the symbol. When users click on the basket the cards show up with what they've collected and if they are the right elements or not (total score inc.) correct - incorrect symbols." Built as: `GameProvider.tsx` (context: `collectedIds`, `attemptedIds`, flight state, basket open state, persisted through `game-storage.ts`, loaded from `localStorage` in a `useEffect` after mount so the first render always matches the server and nothing hydration-mismatches), `FloatingIcon.tsx` (one collectible, absolutely positioned by a deterministic hash of its id, not `Math.random`, real `<button>`), `GameFloatingField.tsx` (drops a slice of `gameItems` into whichever section wraps it, `pointer-events-none` on the wrapper so it never blocks the real content, `pointer-events-auto` on each icon), `GameToast.tsx` (one shared `aria-live` banner showing whatever was just clicked, since there is no longer a single boxed panel), `FlyingIcon.tsx` (the click-to-basket flight, transform and opacity only, measures the basket's rect in an effect rather than during render to satisfy `react-hooks/refs`, renders nothing under `prefers-reduced-motion` or before that measurement lands), `GameBasket.tsx` (fixed bottom right, count badge, scale-bump on collect), `Scorecard.tsx` (opens on basket click, lists every attempted item marked correct/incorrect with a running score). `gameItems` (21 total) is sliced into 7 groups of 3 and one group is dropped into each of 7 sections down the page (intro, who-am-i, camera roll, into-it, experiences, impact, contact) via `GameFloatingField`, so icons read as sprinkled through the whole document rather than boxed into one board. This also carries T-G.4 and T-G.5, folded in as the same build; both those rows are marked done here rather than duplicated. Resolves T-G.10 by keeping `RevealAll` from T-G.6 in place. Build compiles and `npx tsc --noEmit` and `npx eslint src` are both clean (two real lint errors surfaced and were fixed for real, not suppressed blind: `FlyingIcon` was reading `basketRef.current` during render, moved into a `useEffect` plus local state; `GameProvider`'s mount-time localStorage load needed an explicit `eslint-disable react-hooks/set-state-in-effect` block since a one-time hydration-safe load on mount is exactly what that rule's general warning does not apply to). **Note for the next agent:** `npm run build`'s static export step (`EBUSY: resource busy or locked, rmdir .../out`) failed repeatedly while another agent had a concurrent process running against this same working directory. That is a Windows file-lock collision from two agents sharing one checkout, not a code defect. Verified instead via `npx tsc --noEmit`, `npx eslint src`, the dev server's actual HTTP response (`curl`, confirmed the floating icons and their `aria-label`s render, no `GameBoard` reference anywhere), and a browser screenshot. Re-run `npm run build` once nothing else is mid-build in this directory to get a fresh `out/` and re-grep it. |
| T-G.4 | Basket and collection animation | DONE | sonnet-5/session_797907ec | Built as part of T-G.11, see that row for detail and verification. |
| T-G.5 | Hovering scorecard | DONE | sonnet-5/session_797907ec | Built as part of T-G.11, see that row for detail and verification. Renamed in practice to a click-to-open scorecard rather than hover: Ananya's own wording is "when users click on the basket the cards show up", and hover has no equivalent on a phone (T-G.8). |
| T-G.6 | Reveal-all escape hatch | DONE | sonnet-5/session_797907ec-sub, 2026-09-04T00:00Z | Built `src/components/game/RevealAll.tsx`, a `<details>/<summary>` disclosure with no client state, rendered in the intro section of `src/app/page.tsx` (moved there from the retired `#game` section by T-G.11). Lists every item from `src/content/game-items.ts` (label plus reveal, decoys marked ", a lie") with no invented copy. Needs zero JS since it is a native element, so its content is in the server rendered HTML from the start; verified with `grep "Skip the game, show me everything" out/index.html` and `grep "<details" out/index.html` after `npm run build`, both hit, and confirmed the item reveal text (e.g. the running shoes line) is also in the static output. |
...
| T-G.9 | Progress persistence | DONE | sonnet-5/session_797907ec | Folded into `GameProvider.tsx` (see T-G.11) rather than shipped standalone, since the state it persists moved from `GameBoard` into the provider along the way. `src/lib/game-storage.ts` has `loadProgress`/`saveProgress`, both wrapped in try/catch so a thrown or blocked `localStorage` (Safari private mode, quota, policy) just means progress does not persist rather than a broken game. Loaded once after mount in an effect, saved on every change to `collectedIds`/`attemptedIds` once the initial load has happened (guarded by a `hasLoaded` ref so the save effect does not immediately overwrite a freshly loaded value with the empty initial state). |
