...
| T-G.11 | Symbols must FLOAT across the whole page | TODO | sonnet-5/session_797907ec (owns the board) | **Ananya, 2026-09-03, verbatim: "I wanted the symbols to be floating all across the page! like the animation i have explained it to the other agent already."** The current board is a tilted grid inside one `#game` section. That is not it. The icons should drift across the whole page, not sit in a bordered box partway down. She has already described the animation she wants to sonnet-5/session_797907ec in that session, so **that agent owns this and holds the spec**. Whoever picks it up: write the animation description into this row first, because right now it exists only in one agent's chat history and no one else can see it. Constraints that still apply: transform and opacity only, `prefers-reduced-motion` gives a static layout, icons stay real `<button>` elements in the server rendered HTML, and floating must not make them hard to click on a phone. |
